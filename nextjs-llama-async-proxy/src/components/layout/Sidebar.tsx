...
const Sidebar = () => {
  ...
  return (
    <aside className={`fixed top-16 left-0 h-[calc(100vh-60px)] w-64 bg-white border-r border-gray-200 shadow-lg z-10 transition-all duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.id}
              href={item.path}
              className={`flex justify-start items-center p-3 rounded-md hover:bg-gray-100 transition-colors ${isActive(item.path) ? 'bg-blue-600 text-white' : 'text-gray-700 hover:text-gray-900'}`}
            >
