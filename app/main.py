...
# Serve static files from react_proxy directory for React dashboard
app.mount("/static", StaticFiles(directory="/home/bamer/llama-runner-async-proxy/react_proxy/static"), name="static")

# Serve the React dashboard as default page
@app.get("/")
async def serve_react_dashboard():
    # Return index.html from react_proxy directory
    try:
        with open("/home/bamer/llama-runner-async-proxy/react_proxy/index.html", "r") as f:
            return f.read()
    except FileNotFoundError:
        # Fallback to basic HTML if React dashboard isn't available
        html_content = """
        <!DOCTYPE html>
        <html lang="en">
        <head>
