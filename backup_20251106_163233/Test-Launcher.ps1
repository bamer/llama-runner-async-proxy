...
$configFiles = @("config.json", "config_prefilled.json", "config_prefilled_enhanced.jsonc")
$foundConfigs = 0
...
if (-not (Test-Path "config.json")) {
    Write-Host "2. ⚙️  Configuration du système:" -ForegroundColor Yellow
    Write-Host "   Copier config_prefilled.json vers config.json et modifier" -ForegroundColor White
    Write-Host ""
}
