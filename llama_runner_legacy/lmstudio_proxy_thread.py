...
async def _fetch_non_streaming_v1_response(
    ...
    if port is None:
        ...
        startup_timeout = _calculate_startup_timeout(model_config_details)
        try:
            ...
            _cleanup_runner_future(proxy_server, model_name)
        ...
        if model_name not in proxy_server._runner_ready_futures or not (hasattr(proxy_server._runner_ready_futures[model_name], 'done') and proxy_server._runner_ready_futures[model_name].done()):
...
def _calculate_startup_timeout(model_config_details: Dict[str, Any]) -> int:
  # ---  \"\"\"Calculate startup timeout based on model size and parameters.\"\"\"
    # Base timeout of 120 seconds
    timeout = 120
    
    # Get model path to estimate size
    model_path = model_config_details.get("model_path", "")
    if model_path and os.path.exists(model_path):
        try:
            file_size_gb = os.path.getsize(model_path) / (1024 ** 3)
            # Add 60 seconds per GB, with a minimum of 120 seconds
            timeout = max(120, int(120 + (file_size_gb * 60)))
        except (OSError, ValueError):
            pass
    
    # Cap at 600 seconds (10 minutes) maximum
    return min(timeout, 600)

def _cleanup_runner_future(proxy_server: Any, model_name: str) -> None:
   # ---   \"\"\"Clean up runner future to prevent memory leaks.\"\"\"
    if model_name in proxy_server._runner_ready_futures:
        future = proxy_server._runner_ready_futures[model_name]
        if not future.done():
            future.cancel()
        del proxy_server._runner_ready_futures[model_name]
...
async def health_check(request: Request):
    ...
    try:
        ...
        running_models = 0
        total_models = len(all_models_config)
        for model_name in all_models_config:
            if is_model_running_callback(model_name):
                running_models += 1
...
async def openai_speech_to_text(request: Request):
    """Function to convert speech to text using faster-whisper"""
...
async def openai_speech_to_text_translate(request: Request):
    """Function to convert speech to text using faster-whisper with translation"""
    # Retrieve the audio service from app state
    audio_service = request.app.state.audio_service
...
class LMStudioProxyServer:
    def __init__(self,
                 ...
                 request_runner_start_callback: Callable[[str], Awaitable[int]], # Updated type hint
        ...
        # The task attribute will hold the asyncio.Task returned by uvicorn.Server.serve()
        # which runs the FastAPI app asynchronously.
        self.task: Optional[asyncio.Task[None]] = None # Updated type hint
        # Dictionary storing futures/tasks for runner startup. Futures/Tasks resolve to the runner's port (int).
        # Keys are model names, values are asyncio.Future[int] (including asyncio.Task[int]).
        self._runner_ready_futures: Dict[str, asyncio.Future[int]] = {} # Updated type hint
    ...
    async def start(self):
        ...
        # Initialize audio service if llama_runner_manager is available
