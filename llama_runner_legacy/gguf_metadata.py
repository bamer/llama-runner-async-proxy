...
numpy_available = False  # Initialize to False
try:
...
gguf_available = False  # Initialize to False
try:
...
def prepare_for_json(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        return {str(k): prepare_for_json(v) for k, v in data.items()}  # type: ignore[arg-type]
    elif isinstance(data, (list, tuple)):
        return [prepare_for_json(item) for item in data]  # type: ignore[arg-type]
    # Ensure np is not None before using its attributes
        ...
        if data.size == 1:
            ...
            try:
                return data.item()  # type: ignore[union-attr]
            except Exception:
                # Fallback to string representation if item() fails
                return str(data)  # type: ignore[arg-type]
        elif data.ndim == 1:
            # Convert 1D arrays to string representation
            try:
                if data.dtype == np.uint8 or data.dtype == np.int8:  # type: ignore[union-attr]
                    # Assume bytes and decode
                    return bytes(data).decode('utf-8', errors='replace')  # type: ignore[arg-type]
                else:
                    # Convert numeric array to string
                    # Use a very large int for max_line_width instead of np.inf
                    return np.array2string(data, separator=', ', max_line_width=1000000)  # type: ignore[arg-type]
            except Exception:
...
def extract_gguf_metadata(model_path: str, model_config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        def get_scalar_metadata(metadata_dict: Dict[str, Any], key: str, default: Any = None) -> Any:
            ...
            # Ensure np is not None before using its attributes
            if numpy_available and np is not None and isinstance(value, (np.ndarray, np.memmap)): # Explicit np is not None  # type: ignore[arg-type]
                if value.size == 1:
                    try:
                        return value.item()  # Extracts the single scalar value  # type: ignore[union-attr]
                    except Exception as e:
                        logging.warning(f"Could not extract scalar item from numpy array for key '{key}': {e}")
                        return default
                elif value.ndim == 1: # 1D array
                    try:
                        if value.dtype == np.uint8 or value.dtype == np.int8: # Byte array  # type: ignore[union-attr]
                            ...
                            return value.tolist()  # Converts to Python list  # type: ignore[union-attr]
                    except Exception as e:
                        logging.warning(f"Could not convert 1D numpy array for key '{key}': {e}")
                        return str(value)  # type: ignore[arg-type] # Fallback to string representation
        ...
        if gguf_available and file_type_val is not None:
            try:
                ...
                if file_type_int is not None and LlamaFileType is not None: # Check LlamaFileType availability (it's initialized to None if import fails)
                    ...
                    except ValueError as ve: # Handles cases where the int value is not a valid member of LlamaFileType
                        logging.warning(f"Integer value {file_type_int} for 'general.file_type' is not a valid LlamaFileType member for {model_path}: {ve}")
                        # Try to get a meaningful name from the gguf library's GGMLQuantizationType if available
                        try:
                            from gguf.constants import GGMLQuantizationType
                            if hasattr(GGMLQuantizationType, '_value2member_map_'):
                                if file_type_int in GGMLQuantizationType._value2member_map_:
                                    quant_name = GGMLQuantizationType(file_type_int).name
                                    quantization = f"IQ_{quant_name}" if quant_name.startswith('IQ') else quant_name
                                else:
                                    quantization = f"Type_{file_type_int}"
                            else:
                                quantization = f"Type_{file_type_int}"
                        except (ImportError, ValueError, AttributeError) as inner_e:
                            logging.warning(f"Could not resolve quantization type {file_type_int} using GGMLQuantizationType: {inner_e}")
        ...
        logging.warning(f"Non-blocking error extracting GGUF metadata from {model_path}: {e}\n{traceback.format_exc()}")
        # --- Add debug logging for the return value on error ---
        logging.debug(f"Metadata extraction failed for {model_path}. Returning minimal metadata structure.")
        # --- End debug logging ---
        # Return a minimal structure instead of None to avoid blocking
        return {
            "id": model_config.get("model_id", os.path.basename(model_path)),
            "object": "model",
            "type": "llm",
            "publisher": "local",
            "arch": "unknown",
...
def get_model_lmstudio_format(model_name: str, model_path: str, model_config: Dict[str, Any], is_running: bool) -> Optional[Dict[str, Any]]:
    ...
    if file_size is None:
        logging.warning(f"Non-blocking error: Could not get size for {model_path}. Cannot use cache.")
        # Return a minimal structure if size retrieval fails
        return {
            "id": model_name if "model_id" not in model_config else model_config["model_id"],
            "object": "model",
            "type": "llm",
    ...
    if cached_metadata:
        ...
        if extracted_metadata:
            ...
            logging.warning(f"Non-blocking failure to extract metadata for {model_name} at {model_path}")
...
def get_all_models_lmstudio_format(models_config: Dict[str, Dict[str, Any]], is_model_running_callback: Any) -> List[Dict[str, Any]]:
    """
...
def get_single_model_lmstudio_format(model_name: str, models_config: Dict[str, Dict[str, Any]], is_model_running_callback: Any) -> Optional[Dict[str, Any]]:
    """
