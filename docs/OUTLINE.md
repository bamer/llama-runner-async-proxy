...
The aim of this project is to create a llama.cpp runner with a configurable proxy interface to allow it to emulate other popular runners (mostly LM Studio and Ollama).
...
Should return the same information as the `models` endpoint, but limited to the model with the given id.

## Llama.cpp runner

Here is a dump from the --help option of llama-server listing all the options that are available.
...
A Python venv has been created in this project (under subdirectory dev-venv). All Python development should be done using packages from this venv. Packages can be installed if needed.
