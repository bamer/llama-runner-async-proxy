...
# 🔥 PHASE 4 CORRECTION : Chemins relatifs pour serveur et modèles
function Start-LlamaServer {
    # Chemins relatifs au projet - CORRIGÉ SELON VOS INSTRUCTIONS
    $serverPath = Join-Path $PSScriptRoot "..\\llama\\llama-server.exe"
    $modelPath = Join-Path $PSScriptRoot "..\\llama\\models"
    
    if (Test-Path $serverPath) {
        Write-MenuLog "Démarrage de llama-server avec modèles depuis $modelPath" "INFO"
        Write-ColorOutput "`n🚀 Démarrage de llama-server avec modèles depuis $modelPath..." "Cyan"
        
        # Utiliser le modèle par défaut depuis la configuration
        $defaultModel = Get-DefaultModelFromConfig
        if (-not $defaultModel) {
            $defaultModel = "JanusCoderV-7B.i1-Q4_K_S.gguf"
        }
        
        # 🔥 CORRECTION : Ajouter l'extension .gguf si nécessaire
        if (-not $defaultModel.EndsWith(".gguf")) {
            $defaultModel = "$defaultModel.gguf"
        }
        
        $fullModelPath = Join-Path $modelPath $defaultModel
        ...
        if (Test-Path $fullModelPath) {
            Start-Process -FilePath $serverPath `
                -ArgumentList "--model `"$fullModelPath`" --jinja -c 0 --host 127.0.0.1 --port 8035" `
                -NoNewWindow
        ...
        Write-ColorOutput "   - Téléchargez les modèles GGUF et placez-les dans '..\\llama\\models\\'" "White"
...
@("logs", "config", "scripts", "models", "tools", "..\\llama", "..\\llama\\models") | ForEach-Object {
...
# 🔥 PHASE 4 - NOUVELLE FONCTIONNALITÉ : Découverte automatique des modèles
function Discover-AvailableModels {
    Write-ColorOutput "`n=== 🔍 DÉCOUVERTE AUTOMATIQUE DES MODÈLES ===" "Cyan"
    Write-ColorOutput "Scanning le répertoire des modèles pour détecter les nouveaux modèles GGUF..." "Yellow"
    
    try {
        $pythonPath = Join-Path $script:ProjectRoot "dev-venv\\Scripts\\python.exe"
        $discoverScript = Join-Path $script:ProjectRoot "scripts\\model_discovery.py"
        
        # Créer le script de découverte s'il n'existe pas
        if (-not (Test-Path $discoverScript)) {
            Write-MenuLog "Script de découverte non trouvé, création d'un script temporaire" "INFO"
...
def main():
    ...
    for model_name, model_config in models.items():
        ...
        print(f"     ⚙️  Paramètres: n_gpu_layers={model_config.get('parameters', {}).get('n_gpu_layers', 'N/A')}")
    
    return 0

if __name__ == "__main__":
    sys.exit(main())
"@ | Out-File $discoverScript -Encoding UTF8
        }
        
        # Exécuter le script de découverte
        Write-ColorOutput "🚀 Exécution du script de découverte..." "Yellow"
        & $pythonPath $discoverScript
...
# 🔥 PHASE 4 - Gestion complète des modèles
...
function List-AvailableGGUFModels {
    ...
    $modelsDir = Join-Path $PSScriptRoot "..\\llama\\models"
    Write-ColorOutput "📁 Répertoire des modèles: $modelsDir" "White"
    
    if (Test-Path $modelsDir) {
        $ggufFiles = Get-ChildItem -Path $modelsDir -Filter "*.gguf" -File
        
        if ($ggufFiles.Count -eq 0) {
            Write-ColorOutput "⚠️  Aucun fichier .gguf trouvé dans le répertoire" "Yellow"
            Write-ColorOutput "💡 CONSEIL: Téléchargez des modèles GGUF et placez-les dans ce répertoire" "Yellow"
        } else {
            Write-ColorOutput "✅ Trouvé $($ggufFiles.Count) fichiers GGUF :" "Green"
        ...
        Write-ColorOutput "💡 CONSEIL: Créez le répertoire '..\\llama\\models' et placez-y vos modèles" "Yellow"
    }
}
...
function Add-NewModel {
    ...
    $modelName = Read-Host "`nNom du modèle (ex: mon-modele-7b) [appuyez sur Entrée pour utiliser le nom du fichier]"
    $modelFile = Read-Host "Nom du fichier GGUF (ex: mon-modele-7b.Q4_K_M.gguf)"
    ...
    $fullModelPath = "..\\llama\\models\\$modelFile"
    
    # Vérifier si le fichier existe
    $absModelPath = Join-Path $PSScriptRoot $fullModelPath
...
function Set-ModelParameters {
    param($modelsConfig, $availableModels)
    
    Write-ColorOutput "`n=== ⚙️  MODIFIER PARAMÈTRES D'UN MODÈLE ===" "Cyan"
    
    if ($availableModels.Count -eq 0) {
        Write-ColorOutput "⚠️  Aucun modèle disponible" "Yellow"
        return
    }
    
    Write-Host "`nModèles disponibles :" -ForegroundColor "White"
    for ($i = 0; $i -lt $availableModels.Count; $i++) {
        Write-Host "   [$($i+1)] $($availableModels[$i])" -ForegroundColor "White"
    }
    
    $choice = Read-Host "`n❓ Numéro du modèle à modifier"
    if ($choice -match "^\d+$" -and $choice -ge 1 -and $choice -le $availableModels.Count) {
        $modelName = $availableModels[$choice-1]
        $modelConfig = $modelsConfig.models.$modelName
        
        Write-ColorOutput "`n🔧 Modification du modèle: $modelName" "Yellow"
        Write-ColorOutput "   📁 Chemin actuel: $($modelConfig.model_path)" "White"
        Write-ColorOutput "   ⚙️  Couches GPU actuelles: $($modelConfig.parameters.n_gpu_layers)" "White"
        
        $newGPU = Read-Host "`nNouveau nombre de couches GPU (laisser vide pour ne pas changer)"
        if ($newGPU -and $newGPU -match "^\d+$") {
            $modelConfig.parameters.n_gpu_layers = [int]$newGPU
            Write-ColorOutput "✅ Paramètres mis à jour pour $modelName" "Green"
        }
        ...
        Write-MenuLog "Paramètres modifiés pour le modèle: $modelName" "SUCCESS"
    } else {
        Write-ColorOutput "⚠️  Choix invalide" "Yellow"
    }
}
...
function Set-DefaultModel {
    ...
    Write-ColorOutput "`n=== 🎯 CHANGER MODÈLE PAR DÉFAUT ===" "Cyan"
    ...
    Write-Host "`nModèles disponibles :" -ForegroundColor "White"
    for ($i = 0; $i -lt $availableModels.Count; $i++) {
        $modelInfo = $modelsConfig.models.($availableModels[$i])
        $gpuLayers = $modelInfo.parameters.n_gpu_layers
        Write-Host "   [$($i+1)] $($availableModels[$i])" -ForegroundColor "White" -NoNewline
        Write-Host " (GPU layers: $gpuLayers)" -ForegroundColor "Gray"
    }
    
    $choice = Read-Host "`n❓ Numéro du modèle par défaut"
    if ($choice -match "^\d+$" -and $choice -ge 1 -and $choice -le $availableModels.Count) {
        $newDefault = $availableModels[$choice-1]
        $modelsConfig.default_model = $newDefault
        
        $modelsConfigPath = Join-Path $script:ProjectRoot "config\\models_config.json"
        $modelsConfig | ConvertTo-Json -Depth 5 | Out-File $modelsConfigPath -Encoding UTF8
        
        Write-ColorOutput "✅ Modèle par défaut changé pour '$newDefault'" "Green"
        Write-MenuLog "Modèle par défaut changé: $newDefault" "SUCCESS"
    } else {
        Write-ColorOutput "⚠️  Choix invalide" "Yellow"
    }
}

function Edit-ConfigManually {
    param($configPath)
    
    Write-ColorOutput "`n=== ✏️  ÉDITION MANUELLE DE LA CONFIGURATION ===" "Cyan"
    
    try {
        # Vérifier si notepad est disponible
        if (Get-Command "notepad.exe" -ErrorAction SilentlyContinue) {
            Start-Process "notepad.exe" -ArgumentList $configPath -Wait
            Write-ColorOutput "✅ Fichier édité et sauvegardé" "Green"
        } else {
            Write-ColorOutput "❌ Notepad non disponible" "Red"
            Write-ColorOutput "📁 Chemin du fichier: $configPath" "White"
            Write-ColorOutput "💡 Éditez manuellement ce fichier avec votre éditeur préféré" "Yellow"
        }
    }
    catch {
        Write-ColorOutput "❌ Erreur ouverture fichier: $_" "Red"
        Write-ColorOutput "📁 Chemin du fichier: $configPath" "White"
        Write-ColorOutput "💡 Éditez manuellement ce fichier avec votre éditeur préféré" "Yellow"
    }
}
...
while ($true) {
    ...
    Write-ColorOutput "║    🦙 MENU INTERACTIF - PHASE 4 STABLE    ║" "Cyan"
    Write-ColorOutput "║      Gestion modèles - Corrections appliquées" "Cyan"
    Write-ColorOutput "╚════════════════════════════════════════════╝" "Cyan"
    Write-Host ""
    
    # 🔥 PHASE 4 CORRECTION : Afficher l'état du projet
    ...
    # Afficher le modèle par défaut si disponible
    $defaultModel = Get-DefaultModelFromConfig
    if ($defaultModel) {
