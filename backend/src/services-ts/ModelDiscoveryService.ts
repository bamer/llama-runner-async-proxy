...
export class ModelDiscoveryService extends EventEmitter {
   ...
   */
  async discoverModels(basePath: string, maxDepth = 3): Promise<ModelInfo[]> {
   ...
   */
  _associateMultimodal(models: ModelInfo[]): void {
    // Créer une map de mmproj par répertoire parent
    const mmprojs: Record<string, ModelInfo[]> = {};
    const mainModels: ModelInfo[] = [];
   ...
   */
  async _scanDirectory(dir: string, models: ModelInfo[], currentDepth: number, maxDepth: number): Promise<void> {
   ...
   */
  async _extractGGUFMetadata(filePath: string): Promise<any> {
    try {
   ...
   */
  loadModelConfig(modelsConfig: any, modelName: string): ModelConfig | null {
   ...
   */
  saveModelConfig(modelsConfigPath: string, modelName: string, config: ModelConfig): boolean {
    try {
   ...
   */
  getDefaultParameters(): ModelConfig {
    return {
   ...
   */
  validateModelConfig(config: ModelConfig): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
