...
export class LlamaServerService extends EventEmitter {
   ...
   */
  buildCommand(modelPath: string, config: ModelConfig): string {
    const params: string[] = [];
    ...
    // Ensure numeric parameters are properly handled to avoid type errors
   ...
   */
  async startModel(modelName: string, config: ModelConfig): Promise<{ success: boolean; modelName: string; pid: number }> {
   ...
   */
  stopModel(modelName: string): { success: boolean; modelName: string } {
   ...
   */
  getModelStatus(modelName: string): { modelName: string; status: string; pid?: number; uptime?: number; config?: ModelConfig } {
    const entry = this.processes.get(modelName);
    if (!entry) {
   ...
   */
  getAllRunningModels(): Array<{ name: string; status: string; uptime: number }> {
    const models = [];
   ...
   */
  private _updateProcessStatus(modelName: string, status: string): void {
    const event = {
      type: 'model:status',
      modelName,
      status,
