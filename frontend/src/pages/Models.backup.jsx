...
import ParametersCategoryList from '../components/ParametersCategoryList';
...
const ModelsPage = () => {
  ...
  const selectModel = (model) => {
    ...
    setEditingParams({
      ...model,
      models_dir: model.models_dir || modelsPath || (model.path ? model.path.substring(0, model.path.lastIndexOf('/')) : ''),
    });
  };
  ...
  const startModel = async (modelName) => {
    try {
      await axios.post(`/api/v1/llama/models/${modelName}/start`);
  ...
  const stopModel = async (modelName) => {
    try {
      await axios.post(`/api/v1/llama/models/${modelName}/stop`);
      setLoadedModel(null);
  ...
  return (
    <div style={styles.pageLayout}>
      <div style={styles.mainContent}>
        ...
        <div style={styles.tabs}>
          ...
          <button style={{...styles.tabBtn, ...(selectedTab === 'tuning' ? styles.tabActive : {})}} onClick={() => setSelectedTab('tuning')}>
            Fine-Tune
          <button style={{...styles.tabBtn, ...(selectedTab === 'advanced' ? styles.tabActive : {})}} onClick={() => setSelectedTab('advanced')}>
        </div>
              ...
              <div style={styles.discoveredList}>
                ...
                <div style={styles.discoveredGrid}>
                  {discoveredModels.map((model, idx) => (
                    <div key={idx} style={{...styles.discoveredCard, opacity: selectedDiscoveredModels.has(idx) ? 1 : 0.6, borderWidth: selectedDiscoveredModels.has(idx) ? '2px' : '1px', borderColor: selectedDiscoveredModels.has(idx) ? 'var(--color-primary)' : 'var(--border-color)', cursor: 'pointer'}} onClick={() => toggleDiscoveredModel(idx)}>
      ...
      {selectedModel && (
        <aside style={styles.configSidebar}>
          <div style={styles.sidebarHeader}>
            <h3>⚙️ Configure</h3>
            <button style={{...styles.btn, ...styles.btnSecondary, padding: '0.5rem'}} onClick={() => setSelectedModel(null)}>
              ✕
            </button>
          </div>
          <div style={styles.sidebarContent}>
            <div style={styles.modelName}>{selectedModel.name}</div>
            <div style={styles.configForm}>
              <div style={styles.paramRow}>
              ...
              <div style={styles.paramRow}>
                ...
                <ParameterControl label="Min-P" type="number" step="0.01" min="0" max="1" value={editingParams.min_p} onChange={(v) => setEditingParams({...editingParams, min_p: parseFloat(v)})} />
                <ParameterControl label="Parallel" type="number" min="1" value={editingParams.parallel} onChange={(v) => setEditingParams({...editingParams, parallel: parseInt(v)})} />
              </div>
              ...
              <div style={styles.checkboxRow}>
                <ParameterControl label="No MMap" type="checkbox" checked={editingParams.no_mmap} onChange={(v) => setEditingParams({...editingParams, no_mmap: v})} />
                <ParameterControl label="KV Unified" type="checkbox" checked={editingParams.kv_unified} onChange={(v) => setEditingParams({...editingParams, kv_unified: v})} />
                <ParameterControl label="No Warmup" type="checkbox" checked={editingParams.no_warmup} onChange={(v) => setEditingParams({...editingParams, no_warmup: v})} />
              </div>

              <div style={styles.checkboxRow}>
                <ParameterControl label="CPU MOE" type="checkbox" checked={editingParams.cpu_moe} onChange={(v) => setEditingParams({...editingParams, cpu_moe: v})} />
                <ParameterControl label="Jinja" type="checkbox" checked={editingParams.jinja} onChange={(v) => setEditingParams({...editingParams, jinja: v})} />
                <ParameterControl label="Log Colors" type="checkbox" checked={editingParams.log_colors} onChange={(v) => setEditingParams({...editingParams, log_colors: v})} />
              </div>

              <div style={styles.checkboxRow}>
                <ParameterControl label="GGML CUDA Unified Memory" type="checkbox" checked={editingParams.ggml_cuda_enable_unified_memory} onChange={(v) => setEditingParams({...editingParams, ggml_cuda_enable_unified_memory: v})} />
                <ParameterControl label="TS Enabled" type="checkbox" checked={editingParams.ts_enabled} onChange={(v) => setEditingParams({...editingParams, ts_enabled: v})} />
                <ParameterControl label="Cache Reuse" type="number" min="0" value={editingParams.cache_reuse} onChange={(v) => setEditingParams({...editingParams, cache_reuse: parseInt(v)})} />
              </div>

              {selectedModel && selectedModel.multimodal && selectedModel.multimodal.length > 0 && (
                <div style={{...styles.paramRow, borderTop: '1px solid var(--border-color)', paddingTop: '1rem', marginTop: '1rem'}}>
                  <div style={{gridColumn: '1 / -1'}}>
                    <h4 style={{marginBottom: '0.5rem'}}>🎥 Multimodal</h4>
              ...
              <div style={styles.sidebarButtons}>
                <button style={{...styles.btn, ...styles.btnPrimary, width: '100%'}} onClick={saveModelConfig}>💾 Save</button>
                <button style={{...styles.btn, ...styles.btnSecondary, width: '100%'}} onClick={() => setSelectedModel(null)}>✕ Close</button>
              </div>
            </div>
          </div>
        </aside>
...
const styles = {
  pageLayout: { display: 'flex', minHeight: '100vh', backgroundColor: 'var(--bg-primary)' },
  mainContent: { flex: 1, padding: '2rem', maxWidth: 'calc(100% - 400px)', overflowY: 'auto' },
  ...
  modelsList: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' },
  ...
  btn: { padding: '0.5rem 0.75rem', border: 'none', borderRadius: '0.5rem', cursor: 'pointer', fontSize: '0.75rem', fontWeight: '500', whiteSpace: 'nowrap' },
  ...
  discoveredGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem', marginTop: '1rem' },
  ...
  configSidebar: { width: '400px', backgroundColor: 'var(--bg-secondary)', borderLeft: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', position: 'fixed', right: 0, top: 0, height: '100vh', overflowY: 'auto', zIndex: 100, boxShadow: '-2px 0 8px rgba(0,0,0,0.1)' },
  sidebarHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.5rem', borderBottom: '2px solid var(--color-primary)', position: 'sticky', top: 0, backgroundColor: 'var(--bg-secondary)' },
  sidebarContent: { padding: '1.5rem', flex: 1, overflowY: 'auto' },
  modelName: { fontSize: '1.1rem', fontWeight: 'bold', marginBottom: '1rem', color: 'var(--color-primary)' },
  configForm: { display: 'grid', gap: '1rem' },
  paramRow: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' },
  checkboxRow: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' },
  paramControl: { display: 'flex', flexDirection: 'column', gap: '0.25rem' },
  input: { padding: '0.5rem', border: '1px solid var(--border-color)', borderRadius: '0.5rem', backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-primary)', fontSize: '0.75rem', width: '100%', boxSizing: 'border-box' },
  checkbox: { width: '1.25rem', height: '1.25rem', cursor: 'pointer' },
  sidebarButtons: { display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-color)' },
};
